...
public class CommandController implements CommandExecutor{
	...
	private final static Map<Command, Object> handlers = new HashMap<Command, Object>();
	private final static Map<Command, Method> methods = new HashMap<Command, Method>();
	private final static Map<String, SubCommand> subCommands = new HashMap<String, SubCommand>();
	private final static Map<String, Object> subHandlers = new HashMap<String, Object>();
	private final static Map<String, Method> subMethods = new HashMap<String, Method>();
	 ...
	 */
	public static void registerCommands(Object handler, JavaPlugin plugin){
	...
	public static List<Command> getRegisteredCommands(Object handler){
		List<Command> list = new ArrayList<Command>();
		for(Command cmd : handlers.keySet())
			if(handlers.get(cmd).equals(handler))
				list.add(cmd);
		return list;
	}
	
	public static List<SubCommand> getRegisteredSubCommands(Object handler){
		List<SubCommand> list = new ArrayList<SubCommand>();
		for(String cmd : subHandlers.keySet())
			if(handlers.get(cmd).equals(handler))
				list.add(subCommands.get(cmd));
		return list;
	}
